...
class TetrisEngine:
	...
	def move_down_step(self, accelerated=False):
		...
		if not (self.move_piece("DOWN")):
			...
			if tobedeleted:
				...
				self.score += config.levels_dict[f"level{self.current_level}"]["points_per_line"]*len(tobedeleted)
				self.score.set(
					f"Lines Cleared: {self.score} | Level: {self.current_level}"
				)
	...
	def new_game(self):
		...
		self.score = 0
		self.score.set("No. of Lines Cleared = " + str(self.score))
